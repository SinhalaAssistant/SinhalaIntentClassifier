...
# load dataset
dataframe = pandas.read_csv("data/NewMFCCFeaturesWpadding.csv", header=None)
...
model = Sequential()
model.add(Dense(500 ,input_dim=12337, activation='relu'))   #500   #kernel_regularizer=regularizers.l2(0.1),
...
results = model.fit(X_train,y_train,batch_size=256 , epochs=100, callbacks=callbacklist,class_weight=class_weights, validation_data=(X_test,y_test)); #64
